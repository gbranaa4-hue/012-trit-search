...
**`trit_mcp_server.py`**
Exposes OBSERVE's compressed semantic search as an MCP (Model Context Protocol) server, so AI coding assistants (Claude Code, Claude Desktop, etc.) can call it as a tool directly from a conversation. Wraps the same `SearchEngine` as `trit_app.py` — same model, same compressed index — running headless over stdio (no GUI). Three stable tools exposed by default: `search_code(query, k)`, `query_codebase(query, k)` (token-tight variant — dedup + relevance cutoff + compact format, measured 66.3% fewer tokens than `search_code` on the same queries, see [paper/token_reduction_findings.md](paper/token_reduction_findings.md)), and `index_status()`. Five additional experimental tools (`propose_change`, `apply_and_verify`, and the entanglement family) are known-unstable and only register with `OBSERVE_EXPERIMENTAL=1`; see [OBSERVE.md](OBSERVE.md). Verified working through the real MCP protocol layer (`list_tools()`/`call_tool()`), not just direct function calls — returns identical results/scores to the GUI search. Requires an index to already exist (build one first via `trit_app.py` or `trit_search.py --index`); this server only reads, doesn't build.
```
pip install mcp
